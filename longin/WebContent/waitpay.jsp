...
<link rel="stylesheet" href="css/orderinfo.css" />
	...
	<body>
		...
		<div id="div_all">
			<!--头部-->
			<form action="waitPayServlet.do" method="post">
			<div id="div_head">
	...
	<a href="waitPayServlet.do?reqType=2&pageNo=1&pageSize=4&searchType=1">首页</a>&nbsp;
	</c:if>
	
    <a href="waitPayServlet.do?reqType=2&pageNo=${orderlist.pageNo==1?orderlist.pageNo:orderlist.pageNo-1}&pageSize=4&searchType=1">上一页</a>&nbsp;
	
	<a href="waitPayServlet.do?reqType=2&pageNo=${orderlist.pageNo==orderlist.totalPageSize?orderlist.pageNo:orderlist.pageNo+1}&pageSize=4&searchType=1">下一页</a>&nbsp;
	
	<a href="waitPayServlet.do?reqType=2&pageNo=${orderlist.totalPageSize}&pageSize=4&searchType=1">尾页</a>
	...
	第<select onchange="self.location.href=options[selectedIndex].value">
  		...
  		<c:when test="${orderlist.pageNo==i+1}">
  			<option value="waitPayServlet.do?reqType=2&pageNo=${i+1}&pageSize=4&searchType=1" selected="selected">${i+1}</option>
  		</c:when>
  		<c:otherwise>
  		<option value="waitPayServlet.do?reqType=2&pageNo=${i+1}&pageSize=4&searchType=1" >${i+1}</option>
  		</c:otherwise>	
		</c:choose>
		</c:forEach>
	</select>页
