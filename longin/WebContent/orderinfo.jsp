...
<link rel="stylesheet" href="css/orderinfo.css" />
	...
	<a href="orderServlet.do?reqType=2&pageNo=1&pageSize=4">首页</a>&nbsp;
	</c:if>
	
    <a href="orderServlet.do?reqType=2&pageNo=${orderlist.pageNo==1?orderlist.pageNo:orderlist.pageNo-1}&pageSize=4">上一页</a>&nbsp;
	
	<a href="orderServlet.do?reqType=2&pageNo=${orderlist.pageNo==orderlist.totalPageSize?orderlist.pageNo:orderlist.pageNo+1}&pageSize=4">下一页</a>&nbsp;
	
	<a href="orderServlet.do?reqType=2&pageNo=${orderlist.totalPageSize}&pageSize=4">尾页</a>
	...
	第<select onchange="self.location.href=options[selectedIndex].value">
  		...
  		<c:when test="${orderlist.pageNo==i+1 }">
  			<option value="orderServlet.do?reqType=2&pageNo=${i+1} }&pageSize=4" selected="selected">${i+1}</option>
  		</c:when>
  		<c:otherwise>
  		<option value="orderServlet.do?reqType=2&pageNo=${i+1}&pageSize=4" >${i+1}</option>
  		</c:otherwise>	
		</c:choose>
		</c:forEach>
	</select>页
