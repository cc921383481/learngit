...
<link rel="stylesheet" href="css/orderinfo.css" />
	...
	<body>
		...
		<div id="div_all">
			<!--头部-->
			<form action="shippedServlet.do" method="get">
			<div id="div_head">
	...
	<a href="shippedServlet.do?reqType=2&pageNo=1&pageSize=4&searchType=3">首页</a>&nbsp;
	</c:if>
	
    <a href="shippedServlet.do?reqType=2&pageNo=${orderlist.pageNo==1?orderlist.pageNo:orderlist.pageNo-1}&pageSize=4&searchType=3">上一页</a>&nbsp;
	
	<a href="shippedServlet.do?reqType=2&pageNo=${orderlist.pageNo==orderlist.totalPageSize?orderlist.pageNo:orderlist.pageNo+1}&pageSize=4&searchType=3">下一页</a>&nbsp;
	
	<a href="shippedServlet.do?reqType=2&pageNo=${orderlist.totalPageSize}&pageSize=4&searchType=3">尾页</a>
	...
	第<select onchange="self.location.href=options[selectedIndex].value">
  		...
  		<c:when test="${orderlist.pageNo==i+1 }">
  			<option value="shippedServlet.do?reqType=2&pageNo=${i+1} }&pageSize=4&searchType=3" selected="selected">${i+1}</option>
  		</c:when>
  		<c:otherwise>
  		<option value="shippedServlet.do?reqType=2&pageNo=${i+1}&pageSize=4&searchType=3" >${i+1}</option>
  		</c:otherwise>	
		</c:choose>
		</c:forEach>
	</select>页
